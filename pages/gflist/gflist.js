...
Page({
  ...
  clicksave: function (event) {
    ...
    var index = event.currentTarget.dataset.index;
      wx.setClipboardData({
          data: this.data.listArray[index]['des'],
          success: function() {
              wx.showToast({
                  title: '文案复制成功',
                  icon: 'succes',
              ...
              if (that.data.listArray[index]['type'] == '0') {
                  wx.showToast({
                      icon:'loading',
                      title: '暂无图片和视频',
                      duration: 1000
                  })
              }if (that.data.listArray[index]['type'] == '1') {
                var num = 0;
                var arr = 0;
                num = this.data.listArray[index]['url'].length;
                this.saveImageFunc(this.data.listArray[index]['url'], arr, num);
              }if (that.data.listArray[index]['type'] == '2') {
                  that.saveVideoFunc(that.data.listArray[index]['url'][0]);
              }
          }
      })
  },
...
//       }
  ...
  //下载图片
  saveImageFunc: function (data, iex, num) {
    var data = data;
    var that = this;
    var ind = iex;
    if (ind == num) {
        ...
        if (res.statusCode === 200) {
          wx.saveImageToPhotosAlbum({
            ...
            success(res) {
              wx.hideLoading();
              ind++;
              that.saveImageFunc(data, ind, num);//递归算法下载图片
            },
            fail(res) {
              wx.hideLoading()
            }
